            ...
            char *filename, void *arg, struct tm *tm ATTRIBUTE_UNUSED)
{
    ...
    printf("%s - on_picture_save_command() - %d\n",__FILE__,__LINE__);
        ...
        void *dummy2 ATTRIBUTE_UNUSED, struct tm *currenttime_tm)
{
	...
	printf("event_image_detect() - %d , fullfilename = %s\n",__LINE__,fullfilename);
            ...
            void *dummy2 ATTRIBUTE_UNUSED, struct tm *currenttime_tm)
{
    ...
    if (conf->motion_img) {
        ...
        printf("event_imagem_detect() - %d , fullfilenamem = %s\n",__LINE__,fullfilenamem);
            ...
            void *dummy2 ATTRIBUTE_UNUSED, struct tm *currenttime_tm)
{
    ...
    if (strcmp(cnt->conf.snappath, "lastsnap")) {
        ...
        printf("event_image_snapshot() - %d , fullfilename = %s\n",__LINE__,fullfilename);
            ...
            void *dummy2 ATTRIBUTE_UNUSED, struct tm *currenttime_tm)
{
    ...
    printf("event_ffmpeg_newfile() - %d , newfilename = %s\n",__LINE__,cnt->newfilename);
            ...
            struct tm *currenttime_tm)
{
	...
	//printf("event_ffmpeg_timelapse() - %d\n",__LINE__);
        ...
        //printf("event_ffmpeg_timelapse() - %d , timelapsefilename = %s\n",__LINE__,cnt->timelapsefilename);
        if (cnt->imgs.type == VIDEO_PALETTE_GREY) {
            convbuf = mymalloc((width * height) / 2);
            y = img;
            u = convbuf;
	...
	//printf("cnt->conf.ffmpeg_bps = %d ,  cnt->conf.ffmpeg_vbr = %d \n", \
	//cnt->conf.ffmpeg_bps,cnt->conf.ffmpeg_vbr);
	//cnt->conf.ffmpeg_vbr = 5;
	//
	//ffmpeg *cnt->ffmpeg_timelapse
	...
	//void *cnt->ffmpeg_timelapse->udata 平面的灰度图像
