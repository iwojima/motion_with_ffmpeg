			       ...
			       int *width, int *height)
{
    ...
    while (xioctl(s->fd, VIDIOC_ENUM_FMT, &fmt) != -1) {//获取当前视频设备支持的视频格式
        short int i;
...
static void v4l2_set_fps(src_v4l2_t * s){
    ...
    //printf("----------------------------------------%s - v4l2_set_fps() - %d , s->fps = %d\n",__FILE__,__LINE__,s->fps);
...
static int v4l2_set_mmap(src_v4l2_t * s)
{
    ...
    //申请一个拥有24个缓冲帧的缓冲区
    s->req.count = 24;//MMAP_BUFFERS;
    s->req.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
    s->req.memory = V4L2_MEMORY_MMAP;

    if (xioctl(s->fd, VIDIOC_REQBUFS, &s->req) == -1) {  //开启内存映射或用户指针IO
    ...
    for (b = 0; b < s->req.count; b++) {
        ...
        if (xioctl(s->fd, VIDIOC_QBUF, &s->buf) == -1) {  //把帧放入队列
    ...
    if (xioctl(s->fd, VIDIOC_STREAMON, &type) == -1) { //启动视频采集命令
...
static void v4l2_picture_controls(struct context *cnt, struct video_dev *viddev)
{
	...
	printf("%s - v4l2_picture_controls() - %d\n",__FILE__,__LINE__);
...
//#endif
    ...
    viddev->size_map = 24;
    viddev->v4l_buffers[24] = NULL;
    viddev->v4l_maxbuffer = 24;
            ...
            struct config *conf)
{
    ...
    //printf("%s - v4l2_set_input() - %d freq = %d\n",__FILE__,__LINE__,freq);
    //printf("input = %d , viddev->input = %d , width = %d , viddev->width = %d , height = %d ,  
	//    viddev->height = %d , freq = %d , viddev->freq = %d , tuner_number = %d , \
	    viddev->tuner_number = %d\n",input,viddev->input,width,viddev->width,height,
	  //  viddev->height,freq,viddev->freq,tuner_number,viddev->tuner_number);
    if (input != viddev->input || width != viddev->width || height != viddev->height ||
	    ...
	    printf("%s - v4l2_set_input() - %d counter = %d\n",__FILE__,__LINE__,counter);
...
int v4l2_next(struct context *cnt, struct video_dev *viddev, unsigned char *map, int width, int height)
{
    ...
    //printf("%s  v4l2_next() - %d , s->pframe = %d\n",__FILE__,__LINE__,s->pframe);
    if (s->pframe >= 0) {
    ...
    if (xioctl(s->fd, VIDIOC_DQBUF, &s->buf) == -1) {  //从队列中取出帧
