...
static AVOutputFormat *get_oformat(const char *codec, char *filename)
{
    ...
    } else if (strcmp(codec, "asf") == 0) {
	ext = ".asf";
	of = guess_format("asf",NULL,NULL);
                           ...
                           int width, int height, int rate, int bps, int vbr)
{
    ...
    printf("ffmpeg_open() - %d , oformat = %s\n",__LINE__,ffmpeg->oc->oformat->name);
    ...
    printf("%s - ffmpeg_open() - %d , filename = %s\n",__FILE__,__LINE__,ffmpeg->oc->filename);
    ...
    //AVCodecContext *c 描述编解码器上下文的数据结构，包含了众多编解码器需要的参数信息
    ffmpeg->c     = c = AVSTREAM_CODEC_PTR(ffmpeg->video_st);  //define AVSTREAM_CODEC_PTR(avs_ptr) (avs_ptr->codec)
    ...
    //printf("LIBAVCODEC_BUILD >= 4754 , rate_emu = %d\n",c->rate_emu);
    c->time_base.num = 1;  //设置帧率
    c->time_base.den = rate;
    //c->rate_emu = rate;
    printf("LIBAVCODEC_BUILD >= 4754 , rate_emu = %d\n",c->rate_emu);
#else
    //printf("LIBAVCODEC_BUILD < 4754\n");
    c->frame_rate      = rate;
    c->frame_rate_base = 1;
    printf("LIBAVCODEC_BUILD < 4754 , rate_emu = %d\n",c->rate_emu);
#endif /* LIBAVCODEC_BUILD >= 4754 */
    //printf("%s - frame_rate = %d\n",__LINE__,rate);
    ...
    /* open the codec */
    //printf("ffmpeg_open() - %d , c->name = %s\n",__LINE__,codec->name);
    //
    //用给定的codec(AVCodec)初始化c(AVCodecContext)
    ...
    //AVFrame *pFrameDec Allocate an AVFrame and set its fields to default values.
    ...
    av_write_header(ffmpeg->oc);  //av_write_header(AVFormatContext *ac);
                            ...
                            unsigned char *u, unsigned char *v)
{
    ...
    printf("ffmpeg_put_other_image() - %d , picture->");
    if (picture) {
