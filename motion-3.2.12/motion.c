...
static void process_image_ring(struct context *cnt, unsigned int max_images)
{
    ...
    do {
        ...
        if (cnt->imgs.image_ring[cnt->imgs.image_ring_out].shot < cnt->conf.frame_limit) {  //如果画面有运动的话
            /* Output the picture to jpegs and ffmpeg */
	    printf("process_image_ring() - %d\n",__LINE__);
            event(cnt, EVENT_IMAGE_DETECTED,                                       //调用event_image_detected,捕获运动画面
	...
	//存储为预览图像，只在图像为运动
        if (cnt->imgs.image_ring[cnt->imgs.image_ring_out].flags & IMAGE_MOTION) {
            ...
            if (cnt->new_img & NEWIMG_BEST) { //jpeg图像类型
            ...
            if (cnt->new_img & NEWIMG_CENTER) { //jpeg图像类型
...
static int motion_init(struct context *cnt)
{
    ...
    cnt->video_dev = vid_start(cnt);  //cnt->video_dev获取/dev/video0的文件描述符
    ...
    printf("motion_init() - %d , mask_file = %s\n",__LINE__,cnt->conf.mask_file);
...
static void *motion_loop(void *arg)
{
	...
	//printf("motion.c - %d , frame_buffer_size = %d\n",__LINE__,frame_buffer_size);

        if (cnt->imgs.image_ring_size != frame_buffer_size) 
            image_ring_resize(cnt, frame_buffer_size); //重新size图像的buffersize
	...
	//为在这一秒捕获的frame累加shots
        ...
        /***** MOTION LOOP - IMAGE CAPTURE SECTION *****/
            ...
            //从摄像机获取下一个frame，如果vid_next返回0 则表示正确，我们获取了一张新的图像，非零表示错误
            if (cnt->video_dev >= 0){
                vid_return_code = vid_next(cnt, cnt->current_image->image); //这里的image就是后面保存为图像的buffer
		//int fd = open("/home/stewart/share/1.jpg",O_CREAT|O_RDWR,0);
		//write(fd , cnt->current_image->image , sizeof(cnt->current_image->image));
		//lose(fd);
		//printf("motion_loop() - %d , image = %s\n",__LINE__,cnt->current_image->image);
	    }else{
                vid_return_code = 1; /* Non fatal error */
	    }
	    //printf("motion_loop() - %d , vid_return_code = %d\n",__LINE__,vid_return_code);
		...
		//保存新捕获的原始图像到缓冲，我们不会改变文本和图像的位置
            ...
            */            
                 ...
                 * other way
                 */
                if (vid_return_code == NETCAM_RESTART_ERROR) {  //#define NETCAM_RESTART_ERROR 0x12
                    motion_log(LOG_ERR, 0, "Restarting Motion thread to reinitialize all "
                                           "image buffers");
                    /* Break out of main loop terminating thread 
                     * watchdog will start us again 
                ...
                //累加missing_frame_counter
		//第一missing_frames_timeout秒我们复制以前的原始图像。missing_frames_timeout秒后
		//我们把灰色残差图像放入缓冲区中。如果我们还没有从摄像机接收原始图像，我们径直的
		//灰色残差图像。
	    ...
	    //
            if (cnt->threshold && !cnt->pause) {
                 ...
                 * anyway
                 */
		//如果我们已经检测到的运动，我们要看是否有还是运动，如果有
		//运动，将触发的alg_diff alg_diff_standard
		//
		//如果画面静止，则diffs获取的值为0，如果画面有运动，则diffs将会逐步变大，知道运动停止，再逐步变回0
                if (cnt->detecting_motion || cnt->conf.setup_mode)
                    cnt->current_image->diffs = alg_diff_standard(cnt, cnt->imgs.image_virgin);//alg_diff_standard是较慢的全功能的运动检测算法
                else
                    cnt->current_image->diffs = alg_diff(cnt, cnt->imgs.image_virgin); //alg_diff先调用快速检测算法只看像素的分数
                
		
                //printf("motion_loop() - %d , diffs = %d\n",__LINE__,cnt->current_image->diffs);
	    ...
	    //
	    //如果运动被侦测(cnt->current_image->diffs > cnt->threshold),在增加文本到图像之前，
	    //我们发现运动的中心和大小的坐标,这将被用于文本叠加后添加定位矩形

	    //printf("motion_loop() - %d , threshold = %d\n",__LINE__,cnt->threshold);
            if (cnt->current_image->diffs > cnt->threshold)  //cnt->threshold = 1500
	    ...
	    //printf("motion_loop() - %d , text_left = %d , text_right = %d\n",__LINE__,cnt->conf.text_left,cnt->conf.text_right);
        ...
        /***** MOTION LOOP - ACTIONS AND EVENT CONTROL SECTION *****/
            ...
            if (((cnt->currenttime - cnt->lasttime >= cnt->conf.gap) && cnt->conf.gap > 0) || cnt->makemovie) {
                if (cnt->event_nr == cnt->prev_event || cnt->makemovie) {
                    ...
                    /* Flush image buffer */
		    printf("motion_loop() - %d \n",__LINE__);
                    process_image_ring(cnt, IMAGE_BUFFER_FLUSH);  //Called from 'motion_loop' to save images / send images to movie
                    ...
                    event(cnt, EVENT_ENDMOTION, NULL, NULL, NULL, cnt->currenttime_tm); //执行cnt->conf.on_event_end
	    ...
	    //printf("motion_loop() - %d\n",__LINE__);
            ...
            process_image_ring(cnt, 2);  //Called from 'motion_loop' to save images / send images to movie
    ...
    /***** MOTION LOOP - SNAPSHOT FEATURE SECTION *****/
        ...
        if ((cnt->conf.snapshot_interval > 0 && cnt->shots == 0 &&
            ...
            event(cnt, EVENT_IMAGE_SNAPSHOT, cnt->current_image->image, NULL, NULL, 
                 &cnt->current_image->timestamp_tm); //通知event_image_snapshot保存图像,这里指的不是运动图像，而是一般的静态图像
    ...
    //录像再此
            ...
            if (cnt->current_image->timestamp_tm.tm_min == 0 &&
                ...
                /* If we are daily, raise timelapseend event at midnight */
                    ...
                    if (cnt->current_image->timestamp_tm.tm_hour == 0)
                        event(cnt, EVENT_TIMELAPSEEND, NULL, NULL, NULL, 
                              &cnt->current_image->timestamp_tm);  //event_ffmpeg_timelapseend
                ...
                /* If we are weekly-sunday, raise timelapseend event at midnight on sunday */
                    ...
                    if (cnt->current_image->timestamp_tm.tm_wday == 0 && cnt->current_image->timestamp_tm.tm_hour == 0)
                        event(cnt, EVENT_TIMELAPSEEND, NULL, NULL, NULL, &cnt->current_image->timestamp_tm); //event_ffmpeg_timelapseend
	    ...
	    {
		//printf("motion_loop() - %d , time_current_frame = %d , time_last_frame = %d , \
		  conf.timelapse = %d\n",__LINE__,time_current_frame,time_last_frame,cnt->conf.timelapse);
                event(cnt, EVENT_TIMELAPSE, cnt->current_image->image, NULL, NULL, //event_ffmpeg_timelapse
                      &cnt->current_image->timestamp_tm); // 在这里保存录制的视频
	    }
         ...
         */
	    printf("motion_loop() - %d ffmpeg_timelapse = %d\n",__LINE__,cnt->ffmpeg_timelapse);
    ...
    /***** MOTION LOOP - VIDEO LOOPBACK SECTION *****/
        ...
        if (cnt->conf.setup_mode) {
            event(cnt, EVENT_IMAGE, cnt->imgs.out, NULL, &cnt->pipe, cnt->currenttime_tm);  //event_vid_putpipe
            event(cnt, EVENT_WEBCAM, cnt->imgs.out, NULL, NULL, cnt->currenttime_tm);  //发送图像到页面客户端
        } else {
            event(cnt, EVENT_IMAGE, cnt->current_image->image, NULL, &cnt->pipe, 
                  &cnt->current_image->timestamp_tm);     //event_vid_putpipe

            if (!cnt->conf.webcam_motion || cnt->shots == 1)
                event(cnt, EVENT_WEBCAM, cnt->current_image->image, NULL, NULL, 
                      &cnt->current_image->timestamp_tm);  //发送图像到页面客户端
        }
