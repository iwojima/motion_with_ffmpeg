...
static int vid_v4lx_start(struct context *cnt)
{
    ...
    //printf("%s - %d video_device = %s\n",__FILE__,__LINE__,conf->video_device);
...
int vid_start(struct context *cnt)
{
    ...
    if (conf->netcam_url) {
        ...
        dev = vid_v4lx_start(cnt); //dev为设备文件/dev/video0的文件描述符
#endif    /*WITHOUT_V4L */
...
//vid_next从v4l设备或者netcam获取一个video frame
int vid_next(struct context *cnt, unsigned char *map)
{
     ...
     */
    {
        ...
        pthread_mutex_lock(&vid_mutex);
        dev = viddevs;
        while (dev) {  //遍历设备列表，查找相应设备
            if (dev->fd == cnt->video_dev)
                break;
            dev = dev->next;
	    //
	    //src_v4l2_t *s = (src_v4l2_t *) dev->v4l2_private;
	    //printf("%s - vid_next() - %d s->buf.index = %d\n",__FILE__,__LINE__,s->buf.index);
	    //
        }
        pthread_mutex_unlock(&vid_mutex);
        ...
        if (dev->owner != cnt->threadnr) {
            ...
            dev->frames = conf->roundrobin_frames; //多路视频
        }
	//dev->frames = 24;
#ifdef MOTION_V4L2
	//printf("%s - %d dev->frames = %d\n",__FILE__,__LINE__,dev->frames);
        if (dev->v4l2) {
	    printf("%s - %d dev->frames = %d\n",__FILE__,__LINE__,dev->frames);  //走这里
...
#endif
        if (--dev->frames <= 0) {
	    printf("__________________video_common.c - vid_next() - %d\n",__LINE__);
            dev->owner = -1;
            dev->frames = 0;
            pthread_mutex_unlock(&dev->mutex);
        }
